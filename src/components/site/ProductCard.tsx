...
export function ProductCard({
  ...
  return (
    <article className="lift-hover group flex h-full min-w-0 w-full flex-col overflow-hidden rounded-[0.45rem] border border-border/70 bg-card p-2.5 shadow-soft sm:p-3">
      <Link
        to="/product/$id"
        params={{ id: product.slug }}
        className="img-zoom relative block w-full min-w-0 aspect-square cursor-pointer overflow-hidden rounded-[0.4rem] bg-secondary sm:aspect-[4/5]"
      >
        <img
          src={product.image}
          alt={product.name}
      ...
      <div className="mt-3 flex min-w-0 items-start justify-between gap-3 px-1 sm:mt-4">
        ...
        <div className="shrink-0 text-right">
          ...
          <div className="font-serif text-base text-primary sm:text-lg">
            ₹{fromPrice(product)}
          </div>
        </div>
      </div>

      <div className="mt-3 flex min-w-0 flex-col gap-2 px-1 pb-1 sm:mt-4 sm:flex-row sm:items-center">
        <select
          value={variantId}
          onChange={(e) => setVariantId(e.target.value)}
          aria-label={`Choose an option for ${product.name}`}
          className="min-w-0 w-full flex-1 rounded-full border border-border bg-background px-3 py-2 text-xs text-primary transition-colors focus:border-accent focus:outline-none sm:w-auto"
        >
        ...
        <button
          ...
          disabled={justAdded}
          className={`inline-flex w-full shrink-0 items-center justify-center gap-1 rounded-full px-4 py-2 text-xs uppercase tracking-wider transition-all duration-300 active:scale-95 sm:w-auto ${
            justAdded
              ? "sm:w-[5.5rem] scale-105 bg-accent text-accent-foreground"

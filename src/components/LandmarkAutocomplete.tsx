...
export function LandmarkAutocomplete({
  ...
  placeholder?: string;
  className?: string;
  /** Fires whenever we learn whether Google Places suggestions are actually
   * available (the backend key may not be configured). The parent form
   * uses this to decide whether picking a suggestion should be required —
   * it can't require that if suggestions never load in the first place. */
  ...
  // Let the parent form know whether suggestions are actually available,
  // so it only requires "pick from the dropdown" when a dropdown can
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {configured && !value.placeId && value.text.trim().length >= 3 && !loading && (
        <span className="mt-1 block text-[11px] text-destructive">
          Please pick a location from the suggestions above.
        </span>
      )}
    </div>
  );

...
export async function createOrder(input: {
  ...
  const total = Math.max(0, subtotal + input.deliveryFee + ribbonFee - discountAmount);
...
/** Public lookup for the "Track your order" page — requires both the
 * order number and phone to match, so customers can only ever pull up
 * their own order. Relies on the public select policy added in
 * supabase/migrations/0006_track_order_lookup.sql. */
